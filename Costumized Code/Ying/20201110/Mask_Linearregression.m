...
master_folder = "D:\Scotts Lab\FLIM\Leica SP8\Leica Program\Collaborations\For Ying\RawData\Ying Islet_ROI_Program_Test";
total_mask_num = 9;
...
for idx1 = 3:numel(dir(master_folder))
     ...
     for idx2 = 3:numel(sub2_folder)
         ...
         for idx3 = 3:numel(sub3_folder)
             ...
             G_current = zeros(1,total_mask_num);
             S_current = zeros(1,total_mask_num);
             for mask_num = 1:numel(imfinfo(fullfile(mask_files(1).folder,mask_files(1).name)))
                 [G_cen,S_cen,pixel_sum] = PhaserCen_Mask(file_folder,mask_num);
                 disp(['G:' num2str(G_cen) '; S:' num2str(S_cen) '; Pixels: ' num2str(pixel_sum)]);
                 G_current(mask_num) = G_cen;
                 S_current(mask_num) = S_cen;
             end
             G_data = cat(1,G_data,G_current);
             S_data = cat(1,S_data,S_current);
             condition = [condition;string(sub1_folder(idx1).name)];
             field_No = [field_No;string(sub2_folder(idx2).name)];
...
map_res = 500;
G = nonzeros(G_data(:));S = nonzeros(S_data(:));
...
legend_name = [condition+" "+field_No+" "+ time_point];
legend(legend_name)
...
G1_pro = (S_data + 1/P1(1) * G_data - P1(2))/(P1(1)+ 1/P1(1)); %The G values of the intersection
...
Free_precentage(G_data==0)=0;
%%
% DataTable=table(condition,field_No,time_point,G_data,S_data,Free_precentage);
DataTable=table(condition,field_No,time_point,G_data,S_data,Free_precentage);
filename = 'DataTable.xlsx';
writetable(DataTable,filename,'Sheet',1)
%% Functions
function [G_cen,S_cen,pixel_sum] = PhaserCen_Mask(file_folder,mask_num)
...
for z = 1: numel(mask_files)
    mask_img = imread(fullfile(mask_files(z).folder,mask_files(z).name),mask_num);
