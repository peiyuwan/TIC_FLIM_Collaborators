%% Mask Linearregression with mat file readin. 
...
S_sum = [];
pixel_sum = [];
for i = 1:numel(struct_cell)
    current_struct = struct_cell{i};
    mask_files = dir(fullfile(filefolder(i),'ROI_Mask','*.tif'));
    mask_img = imread(fullfile(mask_files(1).folder,mask_files(1).name),mask_ch(1));
    mask_struct = struct_mask(current_struct,mask_preprocess(mask_img));
    disp(["Mean:" + num2str(mean(current_struct.int(:)))]);
    [G_cen,S_cen,pixel_num] = phasor_cen(mask_struct);
    
    
    
    disp(['G:' num2str(G_cen) '; S:' num2str(S_cen) '; Pixels: ' num2str(pixel_num)]);
    G_sum = cat(1,G_sum,G_cen);
    S_sum = cat(1,S_sum,S_cen);
    pixel_sum = cat(1,pixel_sum,pixel_num);
end
...
filename = 'DataTable_mask2.xlsx';
writetable(DataTable,filename,'Sheet',1)

%% Let's do the histogram
his_axis = [0:0.004:1];
his_sum = zeros(numel(condition_names),numel(his_axis)-1);
...
for i = 1:numel(struct_cell)
    ...
    mask_img = imread(fullfile(mask_files(1).folder,mask_files(1).name),mask_ch(1));
    mask_struct = struct_mask(current_struct,mask_preprocess(mask_img));
    his_bin = phasor_line_his(mask_struct,his_axis,P1);
    
    switch condition(i)
        case condition_names(1)
            his_sum(1,:) = his_sum(1,:)+his_bin;
            
        case condition_names(2)
            his_sum(2,:) = his_sum(2,:)+his_bin;
            
        case condition_names(3)
            his_sum(3,:) = his_sum(3,:)+his_bin;
    end
end
